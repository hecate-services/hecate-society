...
%% A traveller that is entirely fine. Every case here is about the ISLAND's
%% decision, never about the migrant, which is the distinction the module exists
%% to hold.
migrant() -> #{lineage => 7, generation => 3}.

open_island() -> #{border => open, population => 10, max_people => 50}.
...
an_open_island_with_room_admits_test() ->
    ?assertEqual(admit, border:consider(migrant(), open_island())).

a_closed_island_turns_away_and_says_why_test() ->
    Island = (open_island())#{border => closed},
    ?assertEqual({turn_away, closed}, border:consider(migrant(), Island)).

a_full_island_turns_away_and_says_why_test() ->
    Island = (open_island())#{population => 50},
    ?assertEqual({turn_away, full}, border:consider(migrant(), Island)).

%% At the boundary rather than past it: `max_people' is a maximum, so being AT it
%% is being full. An off-by-one here admits one person more than the island said
%% it would hold, for ever, silently.
being_exactly_at_the_maximum_is_full_test() ->
    Island = (open_island())#{population => 50, max_people => 50},
    ?assertEqual({turn_away, full}, border:consider(migrant(), Island)),
    Room = (open_island())#{population => 49, max_people => 50},
    ?assertEqual(admit, border:consider(migrant(), Room)).
...
closed_outranks_full_test() ->
    Island = (open_island())#{border => closed, population => 99},
    ?assertEqual({turn_away, closed}, border:consider(migrant(), Island)).
...
an_island_that_declares_nothing_admits_test() ->
    ?assertEqual(admit, border:consider(migrant(), #{})).
...
every_reason_the_rules_emit_is_declared_and_no_more_test() ->
    Reachable = lists:usort([Why || Island <- island_states(),
                                    {turn_away, Why}
                                        <- [border:consider(migrant(), Island)]]),
