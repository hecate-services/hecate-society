...
%% A person who is entirely fine. Every case here is about the ISLAND's decision,
%% never about the person, which is the distinction the module exists to hold.
person() -> #{lineage => 7, generation => 3}.

open_island() -> #{border => open, persons => 10, max_persons => 50}.
...
an_open_island_with_room_admits_test() ->
    ?assertEqual(admit, border:consider(person(), open_island())).

a_closed_island_turns_away_and_says_why_test() ->
    Island = (open_island())#{border => closed},
    ?assertEqual({turn_away, closed}, border:consider(person(), Island)).

a_full_island_turns_away_and_says_why_test() ->
    Island = (open_island())#{persons => 50},
    ?assertEqual({turn_away, full}, border:consider(person(), Island)).

%% At the boundary rather than past it: `max_persons' is a maximum, so being AT it
%% is being full. An off-by-one here admits one person more than the island said it
%% would hold, for ever, silently.
being_exactly_at_the_maximum_is_full_test() ->
    Island = (open_island())#{persons => 50, max_persons => 50},
    ?assertEqual({turn_away, full}, border:consider(person(), Island)),
    Room = (open_island())#{persons => 49, max_persons => 50},
    ?assertEqual(admit, border:consider(person(), Room)).
...
closed_outranks_full_test() ->
    Island = (open_island())#{border => closed, persons => 99},
    ?assertEqual({turn_away, closed}, border:consider(person(), Island)).
...
an_island_that_declares_nothing_admits_test() ->
    ?assertEqual(admit, border:consider(person(), #{})).
