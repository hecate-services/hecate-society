...
%% ==========================================================================
%%
%% A migrant may arrive at the gates perfectly intact and still be turned away.
...
%% the hard way: because the sender let go irrevocably, a refused migrant had
...
%%
%%   the codec         IS THIS A MIGRANT AT ALL? Widths, codes, numbers. Fixed,
%%                     technical, not negotiable, and a failure here is not a
%%                     traveller: there is nothing to hand back.
%%
%%   `consider/2'      WILL WE HAVE IT? A judgement, about a migrant that is
%%                     fine. A refusal here hands the traveller back, intact.
%%
...
%% "we are full" and "we are closed". What it may refuse tomorrow is a stranger
%% who believes the wrong thing, one whose island never reciprocates, one
%% carrying nothing worth having, or whatever else emerges from islands that
...
%% @doc Whether this island will take this migrant.
%%
%% Takes the arriving traveller and the island's own state, and answers with a
%% word rather than a boolean, because "no" without a reason is the fact that
%% tells a sender nothing and leaves an operator guessing.
%%
%% ⚠ THE MIGRANT IS THE FIRST ARGUMENT AND EVERY RULE SO FAR IGNORES IT. That is
...
%% there is. The moment a rule reads the traveller, this system has a politics.
-spec consider(map(), map()) -> verdict().
consider(Migrant, Island) ->
    weighed([fun closed/2, fun crowded/2], Migrant, Island).
...
closed(_Migrant, #{border := closed}) -> {turn_away, closed};
closed(_Migrant, _Island) -> admit.

%% AND A FULL ISLAND CANNOT TAKE ANOTHER, which is less a policy than a fact, but
%% it is still the refusal of an intact traveller rather than a judgement about
%% one.
crowded(_Migrant, #{population := Pop, max_people := Max}) when Pop >= Max ->
    {turn_away, full};
crowded(_Migrant, _Island) ->
    admit.
